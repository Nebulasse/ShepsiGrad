...
const logger = (0, logger_1.getModuleLogger)('AuthController');
/**
...
class AuthController {
    ...
    async register(req, res) {
        try {
            ...
            // Генерируем токены
            const tokens = await auth_service_1.authService.login(userData.email, userData.password);
    ...
    async login(req, res) {
        try {
            ...
            const authResult = await auth_service_1.authService.login(email, password);
            res.status(200).json({
            ...
            if (error instanceof Error && (error.message === 'Неверные учетные данные' ||
    ...
    async resetPassword(req, res) {
        try {
            ...
            const user = await auth_service_1.authService.resetPassword(token, password);
            res.status(200).json({
