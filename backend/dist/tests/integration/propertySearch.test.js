...
describe('Property Search API', () => {
    ...
    beforeAll(async () => {
        ...
        testProperties = await Property_1.Property.insertMany(properties);
    ...
    describe('GET /api/properties', () => {
        it('returns all active properties', async () => {
            const response = await (0, supertest_1.default)(app_1.app).get('/api/properties');
            expect(response.status).toBe(200);
            expect(response.body.properties.length).toBe(4); // Исключая неактивный объект
            expect(response.body.total).toBe(4);
        });
        it('filters properties by city', async () => {
            const response = await (0, supertest_1.default)(app_1.app).get('/api/properties?city=Шепси');
            expect(response.status).toBe(200);
        ...
        it('filters properties by price range', async () => {
            const response = await (0, supertest_1.default)(app_1.app).get('/api/properties?minPrice=3000&maxPrice=10000');
            expect(response.status).toBe(200);
            expect(response.body.properties.length).toBe(2);
            expect(response.body.properties.every((p) => p.price >= 3000 && p.price <= 10000)).toBe(true);
        });
        it('filters properties by type', async () => {
            const response = await (0, supertest_1.default)(app_1.app).get('/api/properties?type=apartment');
            expect(response.status).toBe(200);
            expect(response.body.properties.length).toBe(2);
            expect(response.body.properties.every((p) => p.type === 'apartment')).toBe(true);
        });
        it('filters properties by number of bedrooms', async () => {
            const response = await (0, supertest_1.default)(app_1.app).get('/api/properties?minBedrooms=3');
            expect(response.status).toBe(200);
            expect(response.body.properties.length).toBe(2);
            expect(response.body.properties.every((p) => p.bedrooms >= 3)).toBe(true);
        });
        it('filters properties by amenities', async () => {
            const response = await (0, supertest_1.default)(app_1.app).get('/api/properties?amenities=pool');
            expect(response.status).toBe(200);
        ...
        it('sorts properties by price ascending', async () => {
            const response = await (0, supertest_1.default)(app_1.app).get('/api/properties?sort=price_asc');
            expect(response.status).toBe(200);
        ...
        it('sorts properties by price descending', async () => {
            const response = await (0, supertest_1.default)(app_1.app).get('/api/properties?sort=price_desc');
            expect(response.status).toBe(200);
            expect(response.body.properties.length).toBe(4);
        ...
        it('sorts properties by rating descending', async () => {
            const response = await (0, supertest_1.default)(app_1.app).get('/api/properties?sort=rating_desc');
            expect(response.status).toBe(200);
            expect(response.body.properties.length).toBe(4);
        ...
        it('paginates results correctly', async () => {
            // Первая страница
            const page1 = await (0, supertest_1.default)(app_1.app).get('/api/properties?limit=2&page=1');
            ...
            // Вторая страница
            const page2 = await (0, supertest_1.default)(app_1.app).get('/api/properties?limit=2&page=2');
            expect(page2.status).toBe(200);
            expect(page2.body.properties.length).toBe(2);
            expect(page2.body.page).toBe(2);
            // Проверяем, что объекты на разных страницах не повторяются
            const ids1 = page1.body.properties.map((p) => p.id);
            const ids2 = page2.body.properties.map((p) => p.id);
        ...
        it('returns properties near specified coordinates', async () => {
            ...
            response.body.properties.forEach((property) => {
                const distance = calculateDistance(43.598, 39.149, property.coordinates.latitude, property.coordinates.longitude);
                expect(distance).toBeLessThanOrEqual(1); // 1 км
            });
        });
        it('handles invalid query parameters gracefully', async () => {
            const response = await (0, supertest_1.default)(app_1.app).get('/api/properties?minPrice=invalid');
    ...
    describe('GET /api/properties/:id', () => {
        it('returns a specific property by ID', async () => {
            ...
            const response = await (0, supertest_1.default)(app_1.app).get(`/api/properties/${propertyId}`);
            expect(response.status).toBe(200);
        ...
        it('returns 404 for non-existent property ID', async () => {
            ...
            const response = await (0, supertest_1.default)(app_1.app).get(`/api/properties/${nonExistentId}`);
            expect(response.status).toBe(404);
        });
        it('returns 400 for invalid property ID format', async () => {
            const response = await (0, supertest_1.default)(app_1.app).get('/api/properties/invalid-id');
            expect(response.status).toBe(400);
        });
    });
});
