...
describe('WebSocket Connection Tests', () => {
    ...
    beforeAll(() => {
        ...
        tenantToken = jsonwebtoken_1.default.sign({ id: 'tenant-user-id' }, jwtSecret);
        landlordToken = jsonwebtoken_1.default.sign({ id: 'landlord-user-id' }, jwtSecret);
    });
    beforeEach((done) => {
        // Создаем HTTP сервер для тестов
        httpServer = (0, http_1.createServer)();
        socketService = new socketService_1.SocketService(httpServer);
        // Запускаем сервер на случайном порту
        httpServer.listen(() => {
            const port = httpServer.address().port;
            // Создаем клиентские соединения
            tenantClient = (0, socket_io_client_1.io)(`http://localhost:${port}`, {
                auth: {
                    token: tenantToken,
                    appType: 'tenant'
                }
            });
            landlordClient = (0, socket_io_client_1.io)(`http://localhost:${port}`, {
                auth: {
                    token: landlordToken,
                    appType: 'landlord'
                }
