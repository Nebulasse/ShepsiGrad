...
export async function up() {
  ...
  const { error: tableError } = await supabase.rpc('create_reviews_table');
  ...
  const { error: functionError } = await supabase.rpc('create_average_rating_function');
...
export async function down() {
  ...
  const { error: functionError } = await supabase.rpc('drop_average_rating_function');
  ...
  const { error: tableError } = await supabase.rpc('drop_reviews_table');
